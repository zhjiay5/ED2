...
do ifm=1,ngrids
   ...
   !   ifaterr = ifaterr +1
   ! end if
   if (minval(mphoto_trc3) < 0.1 .or. maxval(mphoto_trc3) > 20.) then                     !!!Jiaying
      write (reason,fmt='(a,1x)') 'Invalid MPHOTO_TRC3, it must be between 0.1 and 20.'   !!!Jiaying
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if
  ...
  if (kw_tree < 15. .or. kw_tree > 15000.) then
   ...
   !   ifaterr = ifaterr +1
   ! end if
   if (minval(gamma_c3) < 0.001 .or. maxval(gamma_c3) > 0.10) then                       !!!Jiaying
      write (reason,fmt='(a,1x)') 'Invalid GAMMA_C3, it must be between 0.001 and 0.1.'  !!!Jiaying
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if
   ...
   !   ifaterr = ifaterr +1
   ! end if
   if (minval(alpha_c3) < 0.001 .or. maxval(alpha_c3) > 1.) then                        !!!Jiaying
      write (reason,fmt='(a,1x)') 'Invalid ALPHA_C3, it must be between 0.001 and 1.'   !!!Jiaying
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if
  ...
  !    ifaterr = ifaterr +1
  ! end if
   if (minval(growthresp) < 0.0 .or. maxval(growthresp) > 1.0) then                   !!!Jiaying
      write (reason,fmt='(a,1x)') 'Invalid GROWTHRESP, it must be between 0 and 1.'   !!!Jiaying
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if
   ...
   !   ifaterr = ifaterr +1
   ! end if
   if (minval(lwidth_bltree) < 0.01 .or. maxval(lwidth_bltree) > 0.50) then                   !!!Jiaying
      write (reason,fmt='(a,1x)') 'Invalid LWIDTH_BLTREE, it must be between 0.01 and 0.50'   !!!Jiaying
      call opspec_fatal(reason,'opspec_misc')
      ifaterr = ifaterr +1
   end if
