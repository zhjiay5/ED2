...
module growth_balive
   ...
   subroutine dbalive_dt(cgrid, tfact)
     ...
     ! use hurricane       , only : hurricane_mortrate     & ! subroutine  !!!Jiaying
     !                            , hurricane_windthrow    & ! subroutine  !!!Jiaying
     !                            , hurricane_defoliate    ! ! subroutine  !!!Jiaying
    ...
    !              call hurricane_defoliate(cpatch,ico,cgrid%met(ipy)%vels,cb_decrement) !!!Jiaying
     ...
     !             !-- Calculate Hurricane-induced windthrow mortality rate ----------------!
     !             call hurricane_mortrate(csite, cpatch,ico,ipft,cgrid%met(ipy)%vels)   !!!Jiaying
      ...
      !         !-- At the end of the day, Apply Hurricane-induced windthrow mortality ----!
      !         call hurricane_windthrow(csite,ipa)   !!!Jiaying
   ...
   subroutine get_c_xfers(csite,ipa,ico,carbon_balance,green_leaf_factor,tr_bleaf,tr_broot &
      ...
      use pft_coms      , only : q            & ! intent(in)
                               , qsw          & ! intent(in)
                               , phenology    & ! intent(in)  !!!Jiaying
                               , agf_bs       ! ! intent(in)
      use ed_misc_coms  , only : current_time ! ! intent(in)  !!!Jiaying
      ...
      if (first_time) then
         if (printout) then
            ...
            write (unit=66,fmt='(22(a,1x))')                                               &
              ...
              ,'PHEN_STAT_IN','PHN_STAT_OUT','  FLUSH_TIME' &
              ,'    BSTORAGE','          CB',' AVAILABLE_C','      ELONGF'  &
              ,'  GREEN_LEAF',               ' DELTA_BLEAF',' DELTA_BROOT','   DELTA_BSA'  &
      ...
      if (printout) then
         ...
         write(unit=66,fmt='(7(i12,1x),1(11x,l1,1x),5(f12.6,1x),             9(f12.8,1x))')&
               current_time%year,current_time%month,current_time%date,ipft,phenology(ipft) &
              ,phen_stat_in,cpatch%phenology_status(ico),time_to_flush &
              ,cpatch%bstorage(ico),carbon_balance,available_carbon    &
              ,cpatch%elongf(ico),green_leaf_factor,             delta_bleaf,delta_broot   &
