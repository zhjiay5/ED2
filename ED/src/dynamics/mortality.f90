...
module mortality
   ...
   real function survivorship(new_lu,dist_path,mindbh_harvest,cpatch,ico)
      ...
      use hurricane_coms, only : hurricane_s               & ! intent(in) !!! Jiaying Zhang
                               , hurricane_wind_threshold  & ! intent(in) !!! Jiaying Zhang
                               , hurricane_ndbhclass       & ! intent(in) !!! Jiaying Zhang
                               , hurricane_dbh_class       ! ! intent(in) !!! Jiaying Zhang
      ...
      !----- Local variables. -------------------------------------------------------------!
      integer                     :: ipft
      integer                     :: idbhclss        ! DBH class        !!! Jiaying Zhang
      integer                     :: jpa	     ! patch  ID        !!! Jiaying Zhang
      integer                     :: jco	     ! cohort ID        !!! Jiaying Zhang
      real                        :: n_large_trees   ! # of large trees !!! Jiaying Zhang
      real                        :: n_small_trees   ! # of small trees !!! Jiaying Zhang
      !------------------------------------------------------------------------------------!
      ...
      case (7)                                                              !!!Jiaying Zhang
         !---------------------------------------------------------------------------------!
         !     Hurricane.  Mort depends on forest state, cohort DBH and PFT.!!!Jiaying Zhang
         !---------------------------------------------------------------------------------!
	 ! assign survivorship based on structure	 
         ! ------------------------------------------------------------------------------- !
          do idbhclss=1, hurricane_ndbhclass
          if ( cpatch%dbh(ico) >= hurricane_dbh_class(idbhclss)                           &
                  .and. cpatch%dbh(ico)<hurricane_dbh_class(idbhclss+1) )    then
                    survivorship = hurricane_s((idbhclss-1)*n_pft+ipft)
            write(*,'((a,f4.1),(a,i2),(a,f5.3))')  'dbh=', cpatch%dbh(ico), &
                        '; pft=', ipft, '; survive=',survivorship
